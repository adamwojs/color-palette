 ...
 * @method PaletteQuery orderByUserId($order = Criteria::ASC) Order by the user_id column
 *
 ...
 * @method PaletteQuery groupByUserId() Group by the user_id column
 *
 ...
 *
 * @method PaletteQuery leftJoinUser($relationAlias = null) Adds a LEFT JOIN clause to the query using the User relation
 * @method PaletteQuery rightJoinUser($relationAlias = null) Adds a RIGHT JOIN clause to the query using the User relation
 * @method PaletteQuery innerJoinUser($relationAlias = null) Adds a INNER JOIN clause to the query using the User relation
 *
 ...
 * @method Palette findOneByUserId(int $user_id) Return the first Palette filtered by the user_id column
 *
 ...
 * @method array findByUserId(int $user_id) Return Palette objects filtered by the user_id column
 */
abstract class BasePaletteQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `description`, `user_id` FROM `palette` WHERE `id` = :p0';
        try {
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(PalettePeer::USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(PalettePeer::USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(PalettePeer::USER_ID, $userId, $comparison);
    }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof User) {
            return $this
                ->addUsingAlias(PalettePeer::USER_ID, $user->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(PalettePeer::USER_ID, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Palettes\CoreBundle\Model\UserQuery A secondary query class using the current class as primary query
     */
    public function useUserQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinUser($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'User', '\Palettes\CoreBundle\Model\UserQuery');
    }
